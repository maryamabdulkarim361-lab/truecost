...
async function fetchFromSerpApi(query, storeId, deliveryZip, attempt = 1) {
    var _a;
    (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] fetchFromSerpApi called: query="${query}", store_id="${storeId || 'none'}", delivery_zip="${deliveryZip || 'none'}", attempt=${attempt}`);
    const apiKey = (process.env.SERP_API_KEY || '').trim();
    if (!apiKey) {
        const error = 'SERP_API_KEY not configured';
        (0, safeDiagnostics_1.safeLog)('pricing.error', `[PRICING] ${error}`);
    ...
    (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Making request to SerpAPI (attempt ${attempt})...`);
    try {
        ...
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] SerpAPI response status: ${res.status}`);
        if (!res.ok) {
            const error = 'Pricing provider HTTP failure';
            (0, safeDiagnostics_1.safeLog)('pricing.provider_http', { status: res.status });
            (0, safeDiagnostics_1.safeLog)('pricing.warn', `[PRICING] ${error} (attempt ${attempt}/${MAX_RETRIES})`);
            // Retry on server errors (5xx) or rate limits (429)
            if ((res.status >= 500 || res.status === 429) && attempt < MAX_RETRIES) {
                const delay = INITIAL_RETRY_DELAY_MS * Math.pow(2, attempt - 1);
                (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Retrying in ${delay}ms...`);
                await sleep(delay);
                return fetchFromSerpApi(query, storeId, deliveryZip, attempt + 1);
            }
            return { priceUSD: null, link: null, error };
        }
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Parsing SerpAPI response...`);
        const data = await res.json();
        // Check for SerpAPI errors in response
        if (data.error) {
            const error = 'Pricing provider failure';
            (0, safeDiagnostics_1.safeLog)('pricing.warn', `[PRICING] ${error} for query: ${query}`);
            return { priceUSD: null, link: null, error };
        }
        ...
        if (!Array.isArray(results) || results.length === 0) {
            ...
            (0, safeDiagnostics_1.safeLog)('pricing.warn', `[PRICING] ${error} for query: ${query}`);
            (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Response structure: products=${!!(data === null || data === void 0 ? void 0 : data.products)}, organic_results=${!!(data === null || data === void 0 ? void 0 : data.organic_results)}, total_results=${((_a = data === null || data === void 0 ? void 0 : data.search_information) === null || _a === void 0 ? void 0 : _a.total_results) || 'N/A'}`);
            return { priceUSD: null, link: null, error };
        }
        ...
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Successfully fetched price $${priceUSD} for: ${query}`);
        ...
        if (isAbortError) {
            (0, safeDiagnostics_1.safeLog)('pricing.error', `[PRICING] SerpAPI request timeout (attempt ${attempt}/${MAX_RETRIES})`);
            // Retry on timeout
        ...
        (0, safeDiagnostics_1.safeLog)('pricing.error', `[PRICING] SerpAPI fetch error (attempt ${attempt}/${MAX_RETRIES}):`, error);
...
exports.getHomeDepotPrice = (0, functionSecurity_1.onCall)({
    ...
    var _a;
    (0, safeDiagnostics_1.safeLog)('pricing.log', '[PRICING] Function invoked');
    (0, safeDiagnostics_1.safeLog)('pricing.log', '[PRICING] Request data:', JSON.stringify(req.data));
    try {
        // Log API key status (without exposing the key)
        const apiKeySet = !!process.env.SERP_API_KEY;
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] SERP_API_KEY configured: ${apiKeySet ? 'YES' : 'NO'}`);
        if (!apiKeySet) {
            (0, safeDiagnostics_1.safeLog)('pricing.error', '[PRICING] SERP_API_KEY not found in environment variables');
            (0, safeDiagnostics_1.safeLog)('pricing.error', '[PRICING] Make sure .env file exists in functions/ directory with SERP_API_KEY=...');
            return {
                success: false,
                priceUSD: null,
                link: null,
        ...
        if (!materialName) {
            (0, safeDiagnostics_1.safeLog)('pricing.error', '[PRICING] materialName is required');
            throw new https_1.HttpsError('invalid-argument', 'materialName is required');
        }
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Processing request for: ${materialName}${unit ? ` (${unit})` : ''}`);
        const store = (storeNumber || '3620').toString();
        const key = normalizeKey(materialName, unit);
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Cache key: ${key}, Store: ${store}, Delivery Zip: ${deliveryZip || 'none'}`);
        ...
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Checking cache in Firestore...`);
        // Cache lookup with TTL check
        const cached = await docRef.get();
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Cache lookup complete. Exists: ${cached.exists}`);
        if (cached.exists) {
            const d = cached.data();
            ...
            if (updatedAt) {
                ...
                if (ageMs < CACHE_TTL_MS) {
                    ...
                    (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Cache hit for: ${materialName} (age: ${ageMinutes} minutes)`);
                    ...
                    (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Cache data:`, {
                        hasValidPrice,
                    ...
                    return {
                        ...
                        error: cachedError ? (0, safeDiagnostics_1.safeErrorMessage)(cachedError) : (hasValidPrice ? undefined : 'Unable to find price - cached result'),
                    };
                }
                else {
                    (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Cache expired for: ${materialName} (age: ${Math.round(ageMs / 1000 / 60 / 60)} hours)`);
                }
            }
        }
        // Fetch from SerpAPI with retry logic
        const query = unit ? `${materialName} ${unit}` : materialName;
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Fetching from SerpAPI: ${query} (store_id: ${storeId}, delivery_zip: ${deliveryZip || 'none'})`);
        const startTime = Date.now();
        const { priceUSD, link, error } = await fetchFromSerpApi(query, storeId, deliveryZip);
        const fetchTime = Date.now() - startTime;
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] SerpAPI fetch complete. Price: ${priceUSD}, Error: ${error || 'none'}, Time: ${fetchTime}ms`);
        const success = priceUSD !== null;
        // Log success rate metrics
        (0, safeDiagnostics_1.safeLog)('pricing.log', `[PRICING] Fetch result for "${materialName}": success=${success}, price=${priceUSD}, fetchTime=${fetchTime}ms${error ? `, error=${error}` : ''}`);
        ...
        (0, safeDiagnostics_1.safeLog)('pricing.error', '[PRICING] getHomeDepotPrice error:', e);
        (0, safeDiagnostics_1.safeLog)('pricing.error', '[PRICING] Error stack:', e instanceof Error ? e.stack : 'No stack trace');
        ...
        if (e instanceof https_1.HttpsError) {
            ...
            (0, safeDiagnostics_1.safeLog)('pricing.error', '[PRICING] Throwing HttpsError:', e.code, e.message);
            throw e;
        }
        // For other errors, return a proper error response
        const errorMessage = e instanceof Error ? e.message : 'Unknown error occurred';
        (0, safeDiagnostics_1.safeLog)('pricing.error', '[PRICING] Returning error response:', errorMessage);
        return {
            success: false,
            priceUSD: null,
            link: null,
            error: (0, safeDiagnostics_1.safeErrorMessage)(`Internal error: ${errorMessage}`)
        };
    }
});
